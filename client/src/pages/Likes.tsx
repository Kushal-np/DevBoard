...
const Likes = () => {
  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <Heart className="w-8 h-8 text-text-secondary/40 mb-3" />
  ...
  return (
    <div className="max-w-2xl mx-auto py-4 px-4 space-y-3 bg-background">
      {posts.map((post) => {
        ...
        return (
          <div
            ...
            className="group cursor-pointer rounded-2xl border border-border bg-surface p-4 hover:border-primary/40 hover:shadow-sm transition-all"
          >
            <div className="flex gap-4">
              {post.thumbnailUrl ? (
                <img
                  ...
                  alt={post.title}
                  className="w-20 h-20 rounded-xl object-cover flex-shrink-0 bg-background"
                />
              ) : null}

              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <h3 className="font-semibold text-text truncate group-hover:underline">
                    {post.title}
                  </h3>
                  <span
                    className={
                      post.status === "published"
                        ? "text-[11px] px-2 py-0.5 rounded-full flex-shrink-0 bg-primary/10 text-primary"
                        : "text-[11px] px-2 py-0.5 rounded-full flex-shrink-0 bg-background text-text-secondary"
                    }
                  >
                    {post.status}
                  </span>
